                        ...
                        switch (ActiveEvent.EventType)
                        {
                            ...
                            case "2v2":
                                ...
                                if (player.Level > 0 && player.Level < 50)
                                {
                                    ...
                                    player.GrantLevelProportionalXp(1.1, 0, 0, true);
                                }
                                ...
                                {
                                    //35% xp to next level
                                    player.GrantLevelProportionalXp(0.10, 0, 0, true);
                                }

                                //Give 30k lum
                                if (player.MaximumLuminance != null)
                                {
                                    player.GrantLuminance(9000, XpType.Quest, ShareType.None);
                                }

                                // --- 5 Nexus Writs ---
                                var nexusToken = WorldObjectFactory.CreateNewWorldObject(10006161); // Nexus Writ
                                nexusToken.SetStackSize(5);
                                var nexusCreated = player.TryCreateInInventoryWithNetworking(nexusToken);
                                if (nexusCreated)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(nexusToken));
                                    var msgNexus = new GameMessageSystemChat("You have received 5 Nexus Writs", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msgNexus);
                                }

                                // --- 1 Arena Token ---
                                var arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); // Arena Token
                                arenaTrophy.SetStackSize(1);
                                var arenaTrophyCreated = player.TryCreateInInventoryWithNetworking(arenaTrophy);
                                if (arenaTrophyCreated)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(arenaTrophy));
                                    var msgArena = new GameMessageSystemChat("You have received an Arena Token", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msgArena);
                                }

                                // Use a SINGLE RNG for all chance rolls
                                var rng = Random.Shared; // .NET 6+; otherwise use a static Random instance

                                // --- 20% chance: Spin Reset ---
                                if (rng.NextDouble() <= 0.20)
                                {
                                    var spinz = WorldObjectFactory.CreateNewWorldObject(8782420); // Spin Reset
                                    spinz.SetStackSize(1);
                                    var spinCreated = player.TryCreateInInventoryWithNetworking(spinz);
                                    if (spinCreated)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(spinz));
                                        var msgSpin = new GameMessageSystemChat("You have received a Free Spin", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msgSpin);
                                    }
                                }

                                // --- 50% chance: Uber-Stakes Gambling Gems (x3) ---
                                if (rng.NextDouble() <= 0.50)
                                {
                                    var uberGems = WorldObjectFactory.CreateNewWorldObject(423278); // Uber gems
                                    uberGems.SetStackSize(3);
                                    var gemsCreated = player.TryCreateInInventoryWithNetworking(uberGems);
                                    if (gemsCreated)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(uberGems));
                                        var msgGems = new GameMessageSystemChat("You have received some Uber-Stakes Gambling Gems", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msgGems);
                                    }
                                }

                                // --- 5% chance: Nexus Reward Chest ---
                                if (rng.NextDouble() <= 0.05)
                                {
                                    var chest = WorldObjectFactory.CreateNewWorldObject(13375304); // Nexus Reward Chest
                                    chest.SetStackSize(1);
                                    var chestCreated = player.TryCreateInInventoryWithNetworking(chest);
                                    if (chestCreated)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(chest));
                                        var msgChest = new GameMessageSystemChat("You have received a Nexus Reward Chest", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msgChest);
                                    }
                                }

                                // --- 1 Rookie’s Bloodstained Keys ---
                                var baseKey = WorldObjectFactory.CreateNewWorldObject(480608); // Rookie’s Bloodstained Keys
                                baseKey.SetStackSize(1);
                                var baseKeyCreated = player.TryCreateInInventoryWithNetworking(baseKey);
                                if (baseKeyCreated)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(baseKey));
                                    var msgBaseKey = new GameMessageSystemChat("You have received one of Rookie’s Bloodstained Keys", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msgBaseKey);
                                }

                                // --- 25% chance for 1–3 extra Rookie’s Bloodstained Keys ---
                                if (rng.NextDouble() <= 0.25)
                                {
                                    int bonusCount = rng.Next(1, 4); // 1..3 inclusive
                                    for (int i = 0; i < bonusCount; i++)
                                    {
                                        // Create a NEW object per key
                                        var bonusKey = WorldObjectFactory.CreateNewWorldObject(480608);
                                        bonusKey.SetStackSize(1);

                                        var bonusCreated = player.TryCreateInInventoryWithNetworking(bonusKey);
                                        if (bonusCreated)
                                        {
                                            player.Session.Network.EnqueueSend(new GameMessageCreateObject(bonusKey));
                                            var msgBonus = new GameMessageSystemChat("You have received a bonus Rookie’s Bloodstained Key", ChatMessageType.Broadcast);
                                            player.Session.Network.EnqueueSend(msgBonus);
                                        }
                                    }
                                }
                            ...
                            case "ffa":
                                ...
                                ffaWinnernexustoken.SetStackSize(5);
                                ...
                                if (ffaWinnernexustokenCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received 5 Nexus Tokens", ChatMessageType.Broadcast);
                                ...
                                if (ffaWinner_arenaTrophyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received 3 Arena Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }
                            ...
                            case "group":
                                ...
                                if (player.Level > 0 && player.Level < 50)
                                {
                                    player.GrantLevelProportionalXp(0.50 * rewardMultiplier, 0, 0, true);
                                }
                                else if (player.Level >= 50 && player.Level < 150)
                                {
                                    player.GrantLevelProportionalXp(0.30 * rewardMultiplier, 0, 0, true);
                                }
                                else if (player.Level >= 150)
                                {
                                    player.GrantLevelProportionalXp(0.10 * rewardMultiplier, 0, 0, true);
                                }

                                //Give 20k lum, triple if you stayed alive and aren't fighting your own clan
                                if (player.MaximumLuminance != null)
                                {
                                    player.GrantLuminance(10000 * rewardMultiplier, XpType.Quest, ShareType.None);
                                }
                    ...
                    if (shouldReward)
                    {
                        ...
                        switch (ActiveEvent.EventType)
                        {
                            ...
                            case "2v2":
                                ...
                                if (player.MaximumLuminance != null)
                                {
                                    player.GrantLuminance(1, XpType.Quest, ShareType.None);
                                }
                                ...
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(nexustoken));
                                    var msg = new GameMessageSystemChat($"You have received a Nexus Writ", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 5% chance for 1 Rookie’s Bloodstained Keys
                            ...
                            case "ffa":
                                ...
                                if (loser.FinishPlace == 2)
                                {
                                    ...
                                    if (player.MaximumLuminance != null)
                                    {
                                        player.GrantLuminance(100, XpType.Quest, ShareType.None);
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (player.Level > 0 && player.Level < 50)
                                    {
                                        player.GrantLevelProportionalXp(0.125, 0, 0, true);
                                    }
                                    else if (player.Level >= 50 && player.Level < 150)
                                    {
                                        player.GrantLevelProportionalXp(0.025, 0, 0, true);
                                    }
                                    else if (player.Level >= 150)
                                    {
                                        player.GrantLevelProportionalXp(0.025, 0, 0, true);
                                    ...
                                    if (player.MaximumLuminance != null)
                                    {
                                        player.GrantLuminance(250, XpType.Quest, ShareType.None);
                                    }
                            ...
                            case "group":
                                ...
                                {
                                    player.GrantLevelProportionalXp(0.0375, 0, 0, true);
                                }
                                else if (player.Level >= 150)
                                {
                                    player.GrantLevelProportionalXp(0.025, 0, 0, true);
