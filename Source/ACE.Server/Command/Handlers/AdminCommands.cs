...
namespace ACE.Server.Command.Handlers
{
    public static class AdminCommands
    {
        ...
        public static void HandleModifyServerFloatPropertyy(Session session, params string[] parameters)
        {
            try
            {
                var val = float.Parse(parameters[1], CultureInfo.InvariantCulture);
                if (PropertyManager.ModifyFloat(parameters[0], val))
                    CommandHandlerHelper.WriteOutputInfo(session, "Float property updated!");
                else
                    CommandHandlerHelper.WriteOutputInfo(session, "Unknown float property.");
            }
            catch
            {
                CommandHandlerHelper.WriteOutputInfo(session, "Please input a valid float, e.g. 1.35", ChatMessageType.Help);
            }
        }
