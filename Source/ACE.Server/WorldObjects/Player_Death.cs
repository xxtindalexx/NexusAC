    ...
    partial class Player
    {
        ...
        public List<WorldObject> CalculateDeathItems(Corpse corpse)
        {
            ...
            if (IsPKDeath(corpse.KillerId))
            {
                ...
                if (shouldDropTrophy)
                {
                    var dropItem = WorldObjectFactory.CreateNewWorldObject(10006161); //nexus writ
                    dropItem.SetStackSize(5);
                    dropItems.Add(dropItem);
                    ...
                    // 25% chance: Rookie Key (716008)
                    if (Random.Shared.NextDouble() < 0.25)
                    {
                        var rookieKey = WorldObjectFactory.CreateNewWorldObject(716008);
                        rookieKey.SetStackSize(1);
                        dropItems.Add(rookieKey);
                    }

                    // 10% chance: Event Boss Coin (5002609)
                    if (Random.Shared.NextDouble() < 0.10)
                    {
                        var bossCoin = WorldObjectFactory.CreateNewWorldObject(5002609);
                        bossCoin.SetStackSize(1);
                        dropItems.Add(bossCoin);
                    }
                ...
                if (TownControlLandblocks.IsTownControlLandblock(this.Location.Landblock) && shouldDropTcRewards)
                {
                    ...
                    if (townId.HasValue)
                    {
                        ...
                        {
                            var nexustoken = WorldObjectFactory.CreateNewWorldObject(10006161);
