...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public void HandleActionUseWithTarget(uint sourceObjectGuid, uint targetObjectGuid)
        {
            ...
            {
                #region CUSTOM_INSTANT_GEM_42392881
                // Allow targeted cast + consume for this one WCID only
                if (sourceItem.WeenieClassId == 42392881 && target is Creature)
                {
                    HandleActionCastTargetedSpell(targetObjectGuid, sourceItem.SpellDID ?? 0, sourceItem);
                    TryConsumeFromInventoryWithNetworking(sourceItem, 1); // consume 1
                    return;
                }
                #endregion
        ...
        public void HandleActionUseItem(uint itemGuid)
        {
            ...
            {
                #region CUSTOM_INSTANT_GEM_42392881
                // WCID 42392881: instant cast on current selection (no target cursor)
                if (item.WeenieClassId == 42392881 && item.SpellDID != null)
                {
                    Creature selected = null;
                    if (selectedTarget != null)
                        selected = selectedTarget.TryGetWorldObject() as Creature;

                    if (selected != null)
                    {
                        // Route into the normal UseWithTarget pipeline (range/LOS/PK handled there)
                        HandleActionUseWithTarget(itemGuid, selected.Guid.Full);
                        return;
                    }

                    // For this item, require a selection; do not open the cursor
                    Session.Network.EnqueueSend(new GameEventCommunicationTransientString(Session, "Select a target first."));
                    SendUseDoneEvent();
                    return;
                }
                #endregion
