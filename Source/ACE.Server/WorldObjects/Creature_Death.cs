            ...
            try
            {
                ...
                else //If this is a Conflict boss
                {
                    ...
                    if (DateTime.UtcNow < tcEventDurationExpiredTime)
                    {
                        ...
                        {
                            var tcTrophy = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                            ...
                            if (invCreateResult)
                            {
                                ...
                                var msg = new GameMessageSystemChat($"You have received {trophiesPerAttacker} Nexus Tokens for successfully attacking {town.TownName}.", ChatMessageType.Broadcast);
                    ...
                    else
                    {
                        ...
                        foreach (var player in defenderList)
                        {
                            ...
                            {
                                player.Session.Network.EnqueueSend(new GameMessageCreateObject(tcTrophy));
                                var msg = new GameMessageSystemChat($"You have received {trophiesPerDefender} Nexus Tokens for successfully defending {town.TownName}.", ChatMessageType.Broadcast);
                                player.Session.Network.EnqueueSend(msg);
                            }
                        }
        ...
        public void HandleWorldBossDeath()
        {
            ...
            try
            {
                ...
                foreach (var player in landblockPlayers)
                {
                    ...
                    var invCreateResult = player.TryCreateInInventoryWithNetworking(nexustoken);
                    if (invCreateResult)
                    {
                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(nexustoken));
                        var msg = new GameMessageSystemChat($"You have received 5 Nexus Tokens for participating in the death of the mighty {deadBoss.Name}.", ChatMessageType.Broadcast);
                        player.Session.Network.EnqueueSend(msg);
                    }
                }                        
            }
